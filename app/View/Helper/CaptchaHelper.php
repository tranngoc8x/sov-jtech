    ...
    function render($settings=array()) {
        ...
        switch( $this->settings['type']):
            case 'image':
                ...
                echo $this->Html->image($this->Html->url(array('controller'=>$controller, 'action'=>$action, '?'=> $qstring), true), array('class' => 'creload','vspace'=>2));
               // echo $this->Html->link( $this->settings['reload_txt'], '#', array('class' => 'creload', 'escape' => false));
                echo $this->Form->input( $this->settings['model'].'.'. $this->settings['field'], array('autocomplete'=>'off','div'=>false,"id"=>"security",'label'=> false,'class'=>'inqut_frame'));
            break;
            case 'math':
