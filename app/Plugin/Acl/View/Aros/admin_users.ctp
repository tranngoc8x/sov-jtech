...
foreach($users as $user)
{
    ...
    foreach($roles as $role)
	{
	   ...
	   {
	       echo '  <td> <span class="icone-ok green"></span></td>';
	   }
	   else
	   {
	   	   $title = __d('acl', 'Update the user role');
	       echo '  <td>' . $this->Html->link('<span class="icone-ok gray"></span>', '/admin/acl/aros/update_user_role/user:' . $user[$user_model_name][$user_pk_name] . '/role:' . $role[$role_model_name][$role_pk_name], array('title' => $title, 'alt' => $title, 'escape' => false)) . '</td>';
	   }
	}
